...
| Need | Approach |
|------|----------|
| MariaDB has a function not in the expression library | Custom Expression |
| MariaDB has a column type not in the type system | Custom DataType |
| Python object needs custom serialization | Custom Type Adapter |
| New SQL syntax needs backend-specific formatting | Custom Dialect Mixin |
...
AI Prompt: "How do I add support for a MariaDB-specific function that isn't in the expression library?"
