...
- [测试配置](configuration.md)：MariaDB 特定的测试设置
...
每个后端提供**提供者实现**，将测试连接到其特定数据库。测试逻辑是共享的；只有提供者层在每个后端之间不同。
...
**提供者职责：** 参阅[核心测试套件提供者指南](provider_guide.md)。
...
有关提供者实现指南，请参阅[核心测试套件提供者指南](provider_guide.md)。

## 通用测试策略

有关 DummyBackend 使用和三层测试策略，请参阅[核心后端测试指南](backend_testing.md)。
