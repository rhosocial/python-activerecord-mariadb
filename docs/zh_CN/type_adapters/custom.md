...
虽然 MariaDB 后端提供了开箱即用的类型映射，但在某些场景下您可能需要自定义类型转换逻辑。
...
对于需要后端感知转换的类型（如 JSON、UUID、ENUM、SET），通过类型注册表注册自定义适配器：
...
有关适配器协议的详细信息，请参阅[自定义类型适配器](../customization/custom_adapters.md)。

## 另请参阅

- [类型适配器概述](README.md) — 类型转换架构
- [自定义：自定义类型适配器](../customization/custom_adapters.md) — 适配器注册和协议

💡 *AI 提示词：* "Pydantic 的 field_validator 和 model_validator 之间有什么区别？"
