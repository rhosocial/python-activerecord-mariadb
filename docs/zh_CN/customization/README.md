...
1. **自定义表达式** -- 为数据库特定的 SQL 语法创建新的表达式类
...
- **表达式**通过委托其绑定方言的 `format_*()` 方法生成 SQL
- **DataType**通过委托 `dialect.format_data_type()` 生成 DDL SQL
- **方言**由小型、专注的混合类组合而成（每个混合类为一个功能区域提供 `format_*()` 方法）
...
| 需求 | 方法 |
|------|------|
| MariaDB 有表达式库中没有的函数 | 自定义表达式 |
| MariaDB 有类型系统中没有的列类型 | 自定义 DataType |
| Python 对象需要自定义序列化 | 自定义类型适配器 |
| 新 SQL 语法需要后端特定格式化 | 自定义方言混合类 |
...
AI Prompt: "如何为表达式库中没有的 MariaDB 特定函数添加支持？"
