# tests/rhosocial/activerecord_mariadb_test/feature/backend/adapters/test_enum_adapter_backend.py
"""
MariaDB ENUM type adapter integration tests using real database connection.

This module tests the MariaDB-specific ENUM adapter with actual database operations.
"""
import pytest
import pytest_asyncio
from enum import Enum
from rhosocial.activerecord.backend.impl.mariadb.adapters import MariaDBEnumAdapter
...
class TestMariaDBEnumAdapterBackend:
    """Synchronous tests for MariaDB ENUM adapter with real database."""

    def test_adapter_registered_in_backend(self, mariadb_backend):
        """Test that MariaDBEnumAdapter is registered in backend."""
        from enum import Enum

        adapter = mariadb_backend.adapter_registry.get_adapter(Enum, str)

        assert adapter is not None
        assert isinstance(adapter, MariaDBEnumAdapter)

    def test_string_enum_round_trip(self, mariadb_backend):
    ...
    def test_int_enum_round_trip(self, mariadb_backend):
        """Test integer enum round trip through adapter registry."""
        from enum import Enum

        adapter = mariadb_backend.adapter_registry.get_adapter(Enum, int)
    ...
    def test_enum_with_sql_execution(self, mariadb_backend):
        ...
        mariadb_backend.execute("DROP TEMPORARY TABLE IF EXISTS test_enum_table")

    def test_enum_null_handling_with_database(self, mariadb_backend):
        ...
        mariadb_backend.execute("DROP TEMPORARY TABLE IF EXISTS test_enum_null")

    def test_native_mariadb_enum_type(self, mariadb_backend):
        """Test integration with MariaDB native ENUM column type."""
        mariadb_backend.execute("""
        ...
        mariadb_backend.execute("DROP TEMPORARY TABLE IF EXISTS test_native_enum")


class TestAsyncMariaDBEnumAdapterBackend:
    """Asynchronous tests for MariaDB ENUM adapter with real database."""

    @pytest.mark.asyncio
    async def test_async_adapter_registered_in_backend(self, async_mariadb_backend):
        """Test that MariaDBEnumAdapter is registered in async backend."""
        from enum import Enum

        adapter = async_mariadb_backend.adapter_registry.get_adapter(Enum, str)

        assert adapter is not None
        assert isinstance(adapter, MariaDBEnumAdapter)

    @pytest.mark.asyncio
    async def test_async_string_enum_round_trip(self, async_mariadb_backend):
    ...
    @pytest.mark.asyncio
    async def test_async_int_enum_round_trip(self, async_mariadb_backend):
        """Test integer enum round trip through adapter registry (async)."""
        from enum import Enum

        adapter = async_mariadb_backend.adapter_registry.get_adapter(Enum, int)
    ...
    @pytest.mark.asyncio
    async def test_async_enum_with_sql_execution(self, async_mariadb_backend):
        ...
        await async_mariadb_backend.execute("DROP TEMPORARY TABLE IF EXISTS test_async_enum_table")

    @pytest.mark.asyncio
    async def test_async_enum_null_handling_with_database(self, async_mariadb_backend):
        ...
        await async_mariadb_backend.execute("DROP TEMPORARY TABLE IF EXISTS test_async_enum_null")
