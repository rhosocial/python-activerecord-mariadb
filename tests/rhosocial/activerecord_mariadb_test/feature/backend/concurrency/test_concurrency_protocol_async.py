# tests/rhosocial/activerecord_mariadb_test/feature/backend/concurrency/test_concurrency_protocol_async.py
"""
...
async def concurrent_user_model(async_mariadb_backend_single):
    """Setup ConcurrentUser model with shared backend."""
    backend = async_mariadb_backend_single
...
@pytest_asyncio.fixture(scope="function")
async def concurrent_task_model(async_mariadb_backend_single):
...
@pytest_asyncio.fixture(scope="function")
async def mysql_config(async_mariadb_backend_single):
...
async def test_recommended_pattern_independent_backends(mysql_config):
    ...
    setup_backend = AsyncMariaDBBackend(connection_config=config)
...
async def test_sequential_vs_concurrent_comparison(mysql_config):
    ...
    shared_backend = AsyncMariaDBBackend(connection_config=config)
    ...
    # Verify all records exist
    verify_backend = AsyncMariaDBBackend(connection_config=config)
