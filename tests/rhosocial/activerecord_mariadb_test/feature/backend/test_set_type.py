# tests/rhosocial/activerecord_mariadb_test/feature/backend/test_set_type.py
"""
MySQL SET type support tests.

This module tests MariaDB-specific SET type functionality including:
...
class TestSetTypeProtocol:
    ...
    def test_supports_set_type(self):
        ...
        dialect_55 = MariaDBDialect(version=(5, 5, 0))
        assert dialect_55.supports_set_type()

        dialect_56 = MariaDBDialect(version=(5, 6, 0))
        assert dialect_56.supports_set_type()

        dialect_80 = MariaDBDialect(version=(8, 0, 0))
    ...
    def test_format_set_literal_single_value(self):
        ...
        sql, params = dialect.format_set_literal(
            MariaDBSetLiteralExpression(dialect, ['value1'])
        )
    ...
    def test_format_set_literal_multiple_values(self):
        ...
        sql, params = dialect.format_set_literal(
            MariaDBSetLiteralExpression(dialect, ['value3', 'value1', 'value2'])
        )
    ...
    def test_format_set_literal_empty(self):
        ...
        sql, params = dialect.format_set_literal(
            MariaDBSetLiteralExpression(dialect, [])
        )
    ...
    def test_format_set_literal_with_validation(self):
        ...
        sql, params = dialect.format_set_literal(
            MariaDBSetLiteralExpression(dialect, ['red', 'blue'], column_values)
        )
    ...
    def test_format_set_literal_invalid_value_raises_error(self):
        ...
        with pytest.raises(ValueError, match="Invalid SET values"):
            dialect.format_set_literal(
                MariaDBSetLiteralExpression(dialect, ['red', 'yellow'], column_values)
            )
    ...
    def test_format_set_literal_max_members_exceeded(self):
        ...
        with pytest.raises(ValueError, match="maximum 64 members"):
            dialect.format_set_literal(
                MariaDBSetLiteralExpression(dialect, values)
            )
    ...
    def test_format_find_in_set(self):
        ...
        sql, params = dialect.format_find_in_set(
            MariaDBFindInSetExpression(dialect, 'value1', 'tags')
        )
    ...
    def test_format_find_in_set_different_column(self):
        ...
        sql, params = dialect.format_find_in_set(
            MariaDBFindInSetExpression(dialect, 'active', 'status')
        )
    ...
    def test_format_set_contains_single_value(self):
        ...
        sql, params = dialect.format_set_contains(
            MariaDBSetContainsExpression(dialect, 'tags', ['value1'])
        )
    ...
    def test_format_set_contains_multiple_values(self):
        ...
        sql, params = dialect.format_set_contains(
            MariaDBSetContainsExpression(dialect, 'tags', ['value1', 'value2'])
        )
    ...
    def test_format_set_contains_three_values(self):
        ...
        sql, params = dialect.format_set_contains(
            MariaDBSetContainsExpression(dialect, 'status', ['active', 'pending', 'verified'])
        )
...
class TestAsyncSetTypeProtocol:
    ...
    async def test_async_format_set_literal(self):
        ...
        sql, params = dialect.format_set_literal(
            MariaDBSetLiteralExpression(dialect, ['a', 'b', 'c'])
        )
    ...
    async def test_async_format_find_in_set(self):
        ...
        sql, params = dialect.format_find_in_set(
            MariaDBFindInSetExpression(dialect, 'test', 'column')
        )
    ...
    async def test_async_format_set_contains(self):
        ...
        sql, params = dialect.format_set_contains(
            MariaDBSetContainsExpression(dialect, 'tags', ['a', 'b'])
        )
