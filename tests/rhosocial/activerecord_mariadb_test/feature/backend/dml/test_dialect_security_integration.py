# tests/rhosocial/activerecord_mariadb_test/feature/backend/dml/test_dialect_security_integration.py
"""
Integration tests for MariaDB dialect SQL injection security fixes.

These tests verify that the security fixes work correctly when
SQL is actually executed against a MariaDB database.
"""
import pytest
...
class TestMariaDBDialectSecurityIntegration:
    """Integration tests for MariaDB dialect security."""

    @pytest.fixture
    def test_table_with_special_chars(self, mariadb_backend):
        """Create a test table with special characters in defaults and comments."""
        mariadb_backend.execute("DROP TABLE IF EXISTS test_security_chars")
    ...
    def test_default_string_with_single_quote_insert_and_retrieve(self, mariadb_backend, test_table_with_special_chars):
    ...
    def test_comment_with_single_quote_stored_correctly(self, mariadb_backend, test_table_with_special_chars):
        """Test that COMMENT with single quotes is stored correctly."""
        # Get the column comment
        result = mariadb_backend.fetch_one("""
    ...
    @pytest.fixture
    def test_table_for_data_type(self, mariadb_backend):
        """Create a test table for data type security tests."""
        mariadb_backend.execute("DROP TABLE IF EXISTS test_data_type_security")
        yield "test_data_type_security"
        mariadb_backend.execute("DROP TABLE IF EXISTS test_data_type_security")

    def test_valid_data_type_works(self, mariadb_backend, test_table_for_data_type):
        """Test that valid data types work correctly."""
        # This should succeed with valid data type
        mariadb_backend.execute(f"""
        ...
        result = mariadb_backend.fetch_all(f"DESCRIBE {test_table_for_data_type}")
        rows = list(result)
        assert len(rows) == 2

    def test_malicious_data_type_rejected_at_dialect_level(self, mariadb_backend):
...
@requires_json_table()
class TestMariaDBJSONTableSecurityIntegration:
    """Integration tests for JSON_TABLE security."""

    @pytest.fixture
    def json_table_name(self, mariadb_backend):
        """Create a test table with JSON data."""
        mariadb_backend.execute("DROP TABLE IF EXISTS test_json_security")
        mariadb_backend.execute("DROP TABLE IF EXISTS test_json_data")
        mariadb_backend.execute("""
