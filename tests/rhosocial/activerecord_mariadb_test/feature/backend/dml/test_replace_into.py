# tests/rhosocial/activerecord_mariadb_test/feature/backend/dml/test_replace_into.py
"""
MySQL REPLACE INTO tests.

Tests for the MariaDB-specific REPLACE INTO functionality which deletes
...
class TestMariaDBReplaceInto:
    """Synchronous REPLACE INTO tests for MariaDB backend."""
    ...
    def test_supports_replace_into(self, mariadb_backend):
        """Test that MariaDB dialect supports REPLACE INTO."""
        assert mariadb_backend.dialect.supports_replace_into() is True

    def test_replace_into_single_row_no_conflict(self, mariadb_backend, test_table):
        ...
        result = mariadb_backend.execute(sql, params)
    ...
    def test_replace_into_with_conflict(self, mariadb_backend, test_table):
        ...
        # Get the original ID
        original_row = mariadb_backend.fetch_one(
    ...
    def test_replace_into_multiple_rows(self, mariadb_backend, test_table):
        ...
        # Verify all rows
        rows = mariadb_backend.fetch_all("SELECT * FROM test_replace_into ORDER BY email")
        assert len(rows) == 3
    ...
    def test_replace_into_with_on_conflict_raises_error(self, mariadb_backend, test_table):
    ...
    def test_replace_and_ignore_mutually_exclusive(self, mariadb_backend, test_table):
...
class TestMySQLAsyncReplaceInto:
    """Asynchronous REPLACE INTO tests for MariaDB backend."""
