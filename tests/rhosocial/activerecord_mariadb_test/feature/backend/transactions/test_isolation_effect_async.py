# tests/rhosocial/activerecord_mariadb_test/feature/backend/transactions/test_isolation_effect_async.py
"""
...
with MariaDB backend using async operations.
"""
import pytest
...
async def async_isolation_test_table(async_mariadb_backend):
    """Create a test table for async isolation tests."""
    await async_mariadb_backend.execute("drop table if exists async_isolation_test")
...
@pytest_asyncio.fixture
async def async_mode_test_table(async_mariadb_backend):
    """Create a test table for transaction mode tests."""
    await async_mariadb_backend.execute("drop table if exists async_mode_test")
    await async_mariadb_backend.execute("""
...
@pytest_asyncio.fixture
async def async_combo_test_table(async_mariadb_backend):
    """Create a test table for combination tests."""
    await async_mariadb_backend.execute("drop table if exists async_combo_test")
    await async_mariadb_backend.execute("""
...
@pytest_asyncio.fixture
async def async_nested_test_table(async_mariadb_backend):
    """Create a test table for nested transaction tests."""
    await async_mariadb_backend.execute("drop table if exists async_nested_test")
    await async_mariadb_backend.execute("""
...
class TestAsyncIsolationLevelEffect:
    ...
    @pytest.mark.asyncio
    async def test_read_uncommitted_allows_dirty_reads(self, async_mariadb_backend, async_mariadb_control_backend, async_isolation_test_table):
        ...
        async def transaction1():
            ...
            try:
                async_mariadb_backend.transaction_manager.isolation_level = IsolationLevel.READ_UNCOMMITTED
        ...
        async def transaction2():
            ...
            try:
                async_mariadb_control_backend.transaction_manager.isolation_level = IsolationLevel.READ_UNCOMMITTED
                async with async_mariadb_control_backend.transaction():
    ...
    @pytest.mark.asyncio
    async def test_read_committed_prevents_dirty_reads(self, async_mariadb_backend, async_mariadb_control_backend, async_isolation_test_table):
        ...
        async def transaction1():
            ...
            try:
                async_mariadb_backend.transaction_manager.isolation_level = IsolationLevel.READ_COMMITTED
                async with async_mariadb_backend.transaction():
        ...
        async def transaction2():
            ...
            try:
                async_mariadb_control_backend.transaction_manager.isolation_level = IsolationLevel.READ_COMMITTED
                async with async_mariadb_control_backend.transaction():
                    await async_mariadb_control_backend.execute(
                        "update async_isolation_test set balance = %s where name = %s",
                        (Decimal("200.00"), "user1")
    ...
    @pytest.mark.asyncio
    async def test_repeatable_read_consistency(self, async_mariadb_backend, async_mariadb_control_backend, async_isolation_test_table):
        ...
        async def transaction1():
            ...
            try:
                async_mariadb_backend.transaction_manager.isolation_level = IsolationLevel.REPEATABLE_READ
                async with async_mariadb_backend.transaction():
                    # First read
                    rows1 = await async_mariadb_backend.fetch_all(
                        "select balance from async_isolation_test where name = %s",
                        ("user1",)
                    )
                    ...
                    rows2 = await async_mariadb_backend.fetch_all(
                        "select balance from async_isolation_test where name = %s",
                        ("user1",)
                    )
    ...
    @pytest.mark.asyncio
    async def test_serializable_prevents_phantom_reads(self, async_mariadb_backend, async_mariadb_control_backend, async_isolation_test_table):
        ...
        async def transaction1():
            ...
            try:
                async_mariadb_backend.transaction_manager.isolation_level = IsolationLevel.SERIALIZABLE
                async with async_mariadb_backend.transaction():
                    # First count
                    rows1 = await async_mariadb_backend.fetch_all(
...
class TestAsyncTransactionModeEffect:
    ...
    @pytest.mark.asyncio
    async def test_read_only_mode_allows_reads(self, async_mariadb_backend, async_mode_test_table):
        """Verify READ ONLY mode allows read operations (async)."""
        if not async_mariadb_backend.dialect.supports_read_only_transaction():
            pytest.skip("MySQL version does not support READ ONLY transactions")

        async_mariadb_backend.transaction_manager.transaction_mode = TransactionMode.READ_ONLY
        async with async_mariadb_backend.transaction():
            rows = await async_mariadb_backend.fetch_all("select * from async_mode_test")
            assert len(rows) == 1
            assert rows[0]["name"] == "account1"

    @pytest.mark.asyncio
    async def test_read_only_rejects_writes(self, async_mariadb_backend, async_mode_test_table):
    ...
    @pytest.mark.asyncio
    async def test_read_write_allows_writes(self, async_mariadb_backend, async_mode_test_table):
        """Verify READ WRITE mode allows write operations (async)."""
        async_mariadb_backend.transaction_manager.transaction_mode = TransactionMode.READ_WRITE
...
class TestAsyncTransactionCombination:
    ...
    @pytest.mark.asyncio
    async def test_serializable_with_read_only(self, async_mariadb_backend, async_combo_test_table):
        ...
        async with async_mariadb_backend.transaction():
                rows = await async_mariadb_backend.fetch_all("select * from async_combo_test")
                assert len(rows) == 1

    @pytest.mark.asyncio
    async def test_repeatable_read_with_read_only(self, async_mariadb_backend, async_combo_test_table):
    ...
    @pytest.mark.asyncio
    async def test_no_isolation_level_set_uses_database_default(self, async_mariadb_backend):
    ...
    @pytest.mark.asyncio
    async def test_explicit_isolation_level_sends_set_transaction(self, async_mariadb_backend):
    ...
    @pytest.mark.asyncio
    async def test_isolation_level_cannot_change_during_transaction(self, async_mariadb_backend, async_isolation_test_table):
...
class TestAsyncNestedTransactionsWithIsolation:
    ...
    @pytest.mark.asyncio
    async def test_nested_transaction_with_isolation_level(self, async_mariadb_backend, async_nested_test_table):
        ...
        async with async_mariadb_backend.transaction():
            ...
            sp = await async_mariadb_backend.transaction_manager.savepoint("sp1")
            ...
            await async_mariadb_backend.transaction_manager.rollback_to(sp)

        rows = await async_mariadb_backend.fetch_all("select * from async_nested_test order by id")
        assert len(rows) == 1
