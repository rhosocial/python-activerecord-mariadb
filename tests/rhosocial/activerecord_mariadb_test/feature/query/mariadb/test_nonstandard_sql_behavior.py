# tests/rhosocial/activerecord_mariadb_test/feature/query/mariadb/test_nonstandard_sql_behavior.py
"""
MariaDB-specific tests for non-standard SQL behavior.

This module contains tests that verify MariaDB's lenient handling of SQL statements
that violate the SQL standard. These tests are intentionally placed in the
backend-specific directory (feature/query/mariadb/) rather than in the testsuite
...
async def test_aggregate_with_order_by_no_group_by(async_order_fixtures):
    """
    Test that MariaDB allows ORDER BY in aggregate queries without GROUP BY.
    """
    AsyncUser, AsyncOrder, AsyncOrderItem = async_order_fixtures

    user = AsyncUser(username='mariadb_agg_user', email='mariadb_agg@example.com', age=30)
    await user.save()
...
async def test_exists_with_retained_order_by(async_order_fixtures):
    """
    Test that MariaDB allows exists() on a query with retained ORDER BY.
    """
    AsyncUser, AsyncOrder, AsyncOrderItem = async_order_fixtures

    user = AsyncUser(username='mariadb_exists_user', email='mariadb_exists@example.com', age=30)
    await user.save()
...
def test_group_by_select_star_non_standard(order_fixtures):
    """
    Test that MariaDB allows SELECT * with incomplete GROUP BY columns.
    """
...
async def test_group_by_select_star_non_standard_async(async_order_fixtures):
    """
    Async version: Test that MariaDB allows SELECT * with incomplete GROUP BY columns.
    """
    AsyncUser, AsyncOrder, AsyncOrderItem = async_order_fixtures
