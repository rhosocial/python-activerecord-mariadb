# tests/rhosocial/activerecord_mariadb_test/feature/query/mariadb/test_json_queries.py
"""
MariaDB JSON query tests.

MariaDB has JSON functions but uses LONGTEXT for JSON columns for
...
@pytest.mark.mariadb_json
@requires_json_operations()
def test_mariadb_json_text_operations(json_user_fixture):
    """
    Test JSON operations using TEXT storage in MariaDB.

    MariaDB supports JSON functions, but to maintain compatibility with
