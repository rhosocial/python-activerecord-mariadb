# tests/rhosocial/activerecord_mariadb_test/feature/connection/test_connection_pool_concurrent.py
"""
Concurrent connection pool tests for MariaDB backend.
...
class TestBackendPoolConcurrent:
    """Tests for concurrent operations with BackendPool (MariaDB)."""

    def test_concurrent_connections_are_distinct(self, mariadb_pool: BackendPool):
    ...
    def test_concurrent_operations_no_deadlock(self, mariadb_pool_with_tables: BackendPool):
        """Verify that concurrent complex operations don't cause deadlock."""
        pool = mariadb_pool_with_tables
        num_threads = 4
    ...
    def test_concurrent_query_results_not_mixed(self, mariadb_pool_with_tables: BackendPool):
    ...
    def test_concurrent_transactions_isolated(self, mariadb_pool_with_tables: BackendPool):
    ...
    def test_pool_stress_concurrent_acquire_release(self, mariadb_pool_large: BackendPool):
        """Stress test: rapidly acquire and release connections concurrently."""
        pool = mariadb_pool_large
        num_threads = 5
...
class TestAsyncBackendPoolConcurrent:
    """Tests for concurrent operations with AsyncBackendPool (MariaDB)."""

    @pytest.mark.asyncio
    async def test_concurrent_connections_are_distinct(self, async_mariadb_pool: AsyncBackendPool):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_operations_no_deadlock(self, async_mariadb_pool_with_tables: AsyncBackendPool):
        """Verify that concurrent async operations don't cause deadlock."""
        pool = async_mariadb_pool_with_tables
        num_concurrent = 4
        results: Dict[int, List[Any]] = {}
    ...
    @pytest.mark.asyncio
    async def test_concurrent_query_results_not_mixed(self, async_mariadb_pool_with_tables: AsyncBackendPool):
    ...
    @pytest.mark.asyncio
    async def test_pool_stress_concurrent_acquire_release(self, async_mariadb_pool_large: AsyncBackendPool):
        """Stress test: rapid async acquire and release."""
        pool = async_mariadb_pool_large
        num_tasks = 5
    ...
    @pytest.mark.asyncio
    async def test_concurrent_with_semaphore_limit(self, async_mariadb_pool: AsyncBackendPool):
        """Test concurrent operations when pool size limits cause waiting."""
        pool = async_mariadb_pool
        num_tasks = 10
