...
package for the MariaDB backend.
...
class BasicProviderBase:
    ...
    def get_dialect(self, scenario_name: str = "default"):
        """Return a bare, fully-constructed MariaDB dialect instance.

        Used by the ``feature/basic/ddl`` subtopic (expression/dialect
        contract). ``(10, 6, 0)`` advertises support for the ``IF [NOT]
        EXISTS`` column modifiers (available since MariaDB 10.0.2).
        """
    ...
    def _load_mariadb_schema(self, filename: str) -> str:
        schema_dir = os.path.join(
            os.path.dirname(__file__), "..", "rhosocial", "activerecord_mariadb_test", "feature", "basic", "schema"
        )
...
class BasicSyncProvider(BasicProviderBase, IBasicSyncProvider, WorkerTestProtocol):
    ...
    def setup_type_test_model(self, scenario_name: str) -> Type[ActiveRecord]:
        ...
        if not temp_dialect.supports_json_type():
            pytest.skip(f"JSON type not supported by MariaDB version {actual_version}")
    ...
    def get_worker_connection_params(self, scenario_name: str, fixture_type: str = None) -> dict:
        ...
        backend_class_name = "AsyncMariaDBBackend" if is_async else "MariaDBBackend"
        table_name = "users"
        if fixture_type:
        ...
        return {
            "backend_module": "rhosocial.activerecord.backend.impl.mariadb",
            "backend_class_name": backend_class_name,
            "config_class_module": "rhosocial.activerecord.backend.impl.mariadb.config",
            "config_class_name": "MariaDBConnectionConfig",
    ...
    def get_worker_schema_sql(self, scenario_name: str, table_name: str) -> str:
        ...
        if fn := TABLE_EXPRESSIONS.get(table_name):
            ...
            sql, _ = to_mariadb_ddl_sql(fn(dialect, table_name))
            return sql
        return self._load_mariadb_schema(f"{table_name}.sql")
