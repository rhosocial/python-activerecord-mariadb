...
"""
Concrete implementation of IQueryConnectionProvider for MariaDB backend.
...
class QueryConnectionProvider(IQueryConnectionProvider):
    """
    MariaDB backend implementation for query connection pool context tests.
    """
    ...
    def setup_sync_pool_and_model(self, scenario_name: str) -> Tuple[BackendPool, Type[ActiveRecord]]:
        ...
        pool_config = PoolConfig(
            min_size=1,
            max_size=5,
            backend_factory=lambda: MariaDBBackend(connection_config=config)
        )
        ...
        SyncQueryTestUser.configure(config, MariaDBBackend)
    ...
    async def setup_async_pool_and_model(self, scenario_name: str) -> Tuple[AsyncBackendPool, Type[AsyncActiveRecord]]:
        ...
        pool_config = PoolConfig(
            min_size=1,
            max_size=5,
            backend_factory=lambda: AsyncMariaDBBackend(connection_config=config)
        )
        ...
        await AsyncQueryTestUser.configure(config, AsyncMariaDBBackend)
